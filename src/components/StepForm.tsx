      ...
      {steps.map((step, i) => {
        ...
        return (
          <div key={i} className="contents">
            <div className="flex items-center gap-[10px] flex-1 last:flex-none">
              {/* Step number */}
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center font-barlow-condensed font-extrabold text-[0.95rem] shrink-0 border-2 transition-all duration-300 ${
                  isDone
            ...
            {/* Line between steps */}
            {i < steps.length - 1 && (
              <div className={`flex-1 h-px mx-3 transition-colors duration-300 ${isDone ? "bg-[#22c55e]" : "bg-[rgba(255,255,255,0.08)]"}`} />
            )}
          </div>
        );
      })}
